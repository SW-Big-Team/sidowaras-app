...
<ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
  ...
  <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="{{ route('karyawan.transaksi.index') }}">Riwayat</a></li>
...
<div class="row mb-3">
  <div class="col-12">
    <a href="{{ route('karyawan.transaksi.index') }}" class="btn-back">
    ...
    <div class="card pro-card h-100">
      <div class="card-header trx-header">
        ...
        <div class="trx-header-info">
          ...
          <h5 class="trx-code">{{ $transaksi->no_transaksi }}</h5>
        </div>
      </div>
        ...
        <div class="info-list">
          <div class="info-item">
            ...
            <div class="info-content">
              ...
              <span class="info-value">{{ $transaksi->created_at->format('d M Y, H:i') }} WIB</span>
            </div>
          </div>
          <div class="info-item">
  ...
  <div class="col-lg-8 mb-4">
    <div class="card pro-card">
      <div class="card-header pro-card-header">
        ...
        <span class="items-count">{{ $transaksi->detail->count() }} Item</span>
      </div>
      <div class="card-body p-0">
          ...
          <table class="table pro-table mb-0">
            ...
            <tbody>
              ...
              <tr>
                <td>
                  <div class="product-cell">
                    ...
                    <div class="product-info">
                      <span class="product-name">{{ $item->batch->obat->nama_obat ?? 'Unknown' }}</span>
                      <span class="product-code">{{ $item->batch->kode_batch ?? '-' }}</span>
                    </div>
                  </div>
                </td>
                ...
                <td class="text-end">
                  <span class="price-cell">Rp {{ number_format($item->harga_saat_transaksi, 0, ',', '.') }}</span>
                </td>
                <td class="text-end">
                  <span class="subtotal-cell">Rp {{ number_format($item->sub_total, 0, ',', '.') }}</span>
                </td>
              </tr>
              @endforeach
            </tbody>
        ...
        <div class="summary-section">
          <div class="summary-row total">
            ...
            <span class="summary-value">Rp {{ number_format($transaksi->total_bayar, 0, ',', '.') }}</span>
          </div>
        </div>
      </div>
    </div>
...
.summary-row.total .summary-value { font-size: 1.25rem; font-weight: 700; color: var(--primary); }
</style>
@endpush
@endsection
