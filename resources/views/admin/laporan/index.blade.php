...
@section('content')
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-body p-3">
                        <form method="GET" class="row g-3 align-items-end" id="filterForm">
                            <div class="col-md-3">
                                <label class="form-label text-xs fw-bold text-uppercase text-secondary mb-1">Jenis
                                    Filter</label>
                                <select class="form-select form-select-sm" name="filter_type" id="filterType"
                                    onchange="toggleFilters()">
                                    <option value="daily" {{ request('filter_type') == 'daily' ? 'selected' : '' }}>Harian
                                    </option>
                                    <option value="monthly" {{ request('filter_type') == 'monthly' ? 'selected' : '' }}>
                                        Bulanan</option>
                                    <option value="yearly" {{ request('filter_type') == 'yearly' ? 'selected' : '' }}>Tahunan
                                    </option>
                            ...
                            <div class="col-md-6 filter-group row g-3" id="customFilter" style="display: none;">
                                <div class="col-6">
                                    <label
                                        class="form-label text-xs fw-bold text-uppercase text-secondary mb-1">Dari</label>
                                    <input type="date" name="from" class="form-control form-control-sm"
                                        value="{{ request('from', $from->format('Y-m-d')) }}">
                                </div>
                                <div class="col-6">
                                    <label
                                        class="form-label text-xs fw-bold text-uppercase text-secondary mb-1">Sampai</label>
                            ...
                            <div class="col-md-3">
                                <button type="submit"
                                    class="btn bg-gradient-dark btn-sm mb-0 w-100 d-flex align-items-center justify-content-center gap-1">
                                    <i class="material-symbols-rounded text-sm">search</i> Tampilkan
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="mb-0 text-xxs mt-1 {{ $revenueChange >= 0 ? 'text-success' : 'text-danger' }}">
                                    <span
                                        class="font-weight-bolder">{{ $revenueChange >= 0 ? '+' : '' }}{{ number_format($revenueChange, 1) }}%</span>
                                    dari periode lalu
                                </p>
                            </div>
                    ...
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p
                                    class="mb-0 text-xxs mt-1 {{ $transactionsChange >= 0 ? 'text-success' : 'text-danger' }}">
                                    <span
                                        class="font-weight-bolder">{{ $transactionsChange >= 0 ? '+' : '' }}{{ number_format($transactionsChange, 1) }}%</span>
                                    dari periode lalu
                                </p>
                            </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="mb-0 text-xxs mt-1 {{ $purchasesChange >= 0 ? 'text-danger' : 'text-success' }}">
                                    <span
                                        class="font-weight-bolder">{{ $purchasesChange >= 0 ? '+' : '' }}{{ number_format($purchasesChange, 1) }}%</span>
                                    dari periode lalu
                                </p>
                            </div>
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-header bg-white pb-0 p-3">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="text-xs text-secondary">{{ $from->format('d M Y') }} -
                                    ...
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <div class="text-end">
                                            <span class="badge badge-sm bg-gradient-success">Rp
                                                {{ number_format($medicine->total_revenue, 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                </li>
                            @empty
                            ...
                            @forelse($lowStock as $item)
                                <li class="list-group-item border-0 px-0">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <div class="text-end">
                                            <span
                                                class="badge badge-sm {{ $item->total_stock < ($item->obat->stok_minimum / 2) ? 'bg-gradient-danger' : 'bg-gradient-warning' }}">
                                                <i class="material-symbols-rounded text-xs">warning</i> {{ $item->total_stock }}
                                                tersisa
                                            </span>
                                        </div>
                                    </div>
        ...
        <script>
            ...
            function toggleFilters() {
                ...
                if (type === 'daily') {
                    ...
                    document.getElementById('yearlyFilter').style.display = 'block';
                } else {
    ...
    <style>
        ...
        .bg-soft-primary {
            background: rgba(94, 114, 228, 0.10) !important;
        }
