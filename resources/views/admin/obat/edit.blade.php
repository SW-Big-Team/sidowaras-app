        ...
        <div class="welcome-banner">
            <div class="welcome-content">
                <div class="welcome-text">
                    <span class="greeting-badge"><i class="material-symbols-rounded">edit</i> Edit Obat</span>
                    <h2 class="welcome-title">{{ $obat->nama_obat }}</h2>
                    <p class="welcome-subtitle">Perbarui informasi obat yang sudah terdaftar di sistem apotek.</p>
                </div>
    ...
    <form action="{{ route('admin.obat.update', $obat->id) }}" method="POST" id="formObat">
        ...
        <div class="form-section">
            <div class="section-header">
                <div class="section-icon warning"><i class="material-symbols-rounded">info</i></div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="form-group-modern">
                            ...
                            <div class="input-modern"><i class="material-symbols-rounded input-icon">medication</i><input type="text" name="nama_obat" class="form-control" value="{{ old('nama_obat', $obat->nama_obat) }}" placeholder="Masukkan nama obat" required></div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group-modern">
                            <label class="form-label-modern">Kode Obat</label>
                            <div class="input-modern"><i class="material-symbols-rounded input-icon">tag</i><input type="text" name="kode_obat" class="form-control" value="{{ old('kode_obat', $obat->kode_obat) }}" placeholder="Kode unik"></div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group-modern">
                            <label class="form-label-modern">Stok Minimum</label>
                            <div class="input-modern"><i class="material-symbols-rounded input-icon">warning</i><input type="number" name="stok_minimum" class="form-control" value="{{ old('stok_minimum', $obat->stok_minimum) }}" min="0" placeholder="10"></div>
                ...
                <div class="row g-3">
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-modern select">
                                ...
                                <select name="kategori_id" class="form-control" required>
                                    ...
                                    @foreach($kategori as $k)<option value="{{ $k->id }}" {{ old('kategori_id', $obat->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>@endforeach
                                </select>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-modern select">
                                ...
                                <select name="satuan_obat_id" class="form-control" required>
                                    ...
                                    @foreach($satuan as $s)<option value="{{ $s->id }}" {{ old('satuan_obat_id', $obat->satuan_obat_id) == $s->id ? 'selected' : '' }}>{{ $s->nama_satuan }}</option>@endforeach
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group-modern">
                            ...
                            <div class="input-modern"><i class="material-symbols-rounded input-icon">location_on</i><input type="text" name="lokasi_rak" class="form-control" value="{{ old('lokasi_rak', $obat->lokasi_rak) }}" maxlength="50" placeholder="Contoh: A-01"></div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-8">
                        <div class="form-group-modern">
                            ...
                            @endphp
                            <select name="kandungan_id[]" id="kandunganSelectEdit" class="form-control" multiple>
                                @foreach($kandungan as $k)<option value="{{ $k->id }}" {{ in_array($k->id, old('kandungan_id', $selectedKandungan ?? [])) ? 'selected' : '' }}>{{ $k->nama_kandungan_text }} ({{ $k->dosis_kandungan }})</option>@endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group-modern">
                            ...
                            <div class="input-modern barcode">
                                ...
                                <input type="text" name="barcode" id="barcodeInput" class="form-control" value="{{ old('barcode', $obat->barcode) }}" maxlength="100" placeholder="Scan atau ketik">
                    ...
                    <div class="col-12">
                        <div class="form-group-modern">
                            ...
                            <div class="input-modern textarea"><i class="material-symbols-rounded input-icon">description</i><textarea name="deskripsi" class="form-control" rows="3" placeholder="Deskripsi obat, indikasi, atau keterangan">{{ old('deskripsi', $obat->deskripsi) }}</textarea></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn-pro warning"><i class="material-symbols-rounded">update</i> Update Data Obat</button>
        </div>
    </form>
</div>
...
<div class="modal fade" id="qrScannerModal" tabindex="-1" aria-labelledby="qrScannerLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content pro-modal warning">
...
.btn-scan { position: absolute; right: 4px; top: 50%; transform: translateY(-50%); width: 40px; height: 40px; border-radius: 8px; background: var(--warning); color: white; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; }
...
.pro-modal.warning .modal-header { background: linear-gradient(135deg, #f59e0b, #d97706); }
...
.scan-info { display: flex; align-items: center; gap: 8px; padding: 12px 16px; background: rgba(245,158,11,0.1); border-radius: 10px; color: var(--warning); font-size: 0.85rem; }
...
.select2-container--default.select2-container--focus .select2-selection--multiple { border-color: var(--warning) !important; box-shadow: 0 0 0 3px rgba(245,158,11,0.15) !important; }
.select2-container--default .select2-selection--multiple .select2-selection__choice { background-color: var(--warning) !important; border-color: var(--warning) !important; color: #fff !important; padding: 4px 8px !important; border-radius: 6px !important; }
...
.select2-container--default .select2-results__option--highlighted[aria-selected] { background-color: var(--warning) !important; }
