...
@section('content')
            ...
            <div class="welcome-banner">
                <div class="welcome-content">
                    <div class="welcome-text">
                        <span class="greeting-badge"><i class="material-symbols-rounded">inventory_2</i> Manajemen
                            Stok</span>
                    ...
                    <div class="welcome-stats">
                        <a href="{{ route('pembelian.index') }}" class="stat-pill"><i
                                class="material-symbols-rounded">shopping_cart</i><span>Pembelian</span></a>
                        <a href="{{ route('admin.obat.create') }}" class="stat-pill primary"><i
                                class="material-symbols-rounded">add_circle</i><span>Tambah Obat</span></a>
                    </div>
                </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-xl-3 col-md-6">
            <div class="metric-card primary">
                ...
                <div class="metric-content"><span class="metric-label">Total Item</span>
                    <h3 class="metric-value">{{ $obats->total() }}</h3><span class="metric-subtext">Produk terdaftar</span>
                </div>
        ...
        <div class="col-xl-3 col-md-6">
            <div class="metric-card success">
                ...
                <div class="metric-content"><span class="metric-label">Stok Aman</span>
                    <h3 class="metric-value">{{ $totalAman }}</h3><span class="metric-subtext">Kondisi stabil</span>
                </div>
                <div class="metric-glow"></div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="metric-card warning">
                ...
                <div class="metric-content"><span class="metric-label">Stok Rendah</span>
                    <h3 class="metric-value">{{ $totalStokRendah }}</h3><span class="metric-subtext">Perlu restock</span>
                </div>
                <div class="metric-glow"></div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="metric-card danger">
                ...
                <div class="metric-content"><span class="metric-label">Exp < 30 Hari</span>
                            <h3 class="metric-value">{{ $totalExpSoon }}</h3><span class="metric-subtext">Cek segera</span>
                </div>
                <div class="metric-glow"></div>
            </div>
        </div>
    ...
    <div class="card pro-card">
        <div class="card-header pro-card-header">
            <div class="header-left">
                ...
                <div>
                    <h6 class="header-title">Data Stok Obat</h6>
                    <p class="header-subtitle">{{ $obats->total() }} record</p>
                </div>
            </div>
            <form action="{{ route('stok.index') }}" method="GET" class="filter-form" id="filterForm">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('stok.index') }}" class="btn-clear" title="Hapus Filter"><i
                            class="material-symbols-rounded">close</i></a>
                @endif
            </form>
        </div>
                            ...
                            <tr class="{{ $isExpiredSoon ? 'row-danger' : ($isLow ? 'row-warning' : '') }}">
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <div><span
                                                class="fw-bold d-block">{{ $obat->nama_obat }}</span>@if($obat->lokasi_rak)<span
                                                    class="text-xs text-secondary"><i
                                                        class="material-symbols-rounded text-xs">location_on</i> Rak:
                                                {{ $obat->lokasi_rak }}</span>@endif</div>
                                    </div>
                                </td>
                                ...
                                <td class="text-center">
                                    @if($isExpiredSoon)
                                        <span class="status-badge danger"><i class="material-symbols-rounded">event_busy</i> Exp <
                                                30 Hari</span>
                                    @elseif($isLow)
                                                <span class="status-badge warning"><i class="material-symbols-rounded">warning</i>
                                                    Stok Rendah</span>
                                            @else
                                                <span class="status-badge success"><i
                                                        class="material-symbols-rounded">check_circle</i> Aman</span>
                                            @endif
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('admin.obat.edit', $obat->id) }}" class="action-btn" title="Edit"><i
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-5">
                                    <div class="empty-state"><i class="material-symbols-rounded">inventory_2</i>
    ...
    <div class="legend-bar mt-3">
        ...
        <div class="legend-item"><span class="legend-line danger"></span><span>Kadaluarsa < 30 hari</span>
        </div>
    </div>
    ...
    @push('styles')
        <style>
            ...
            .metric-card.danger .metric-glow {
            ...
            .pro-table tbody tr.row-warning {
                background: rgba(245, 158, 11, 0.04);
            }

            .pro-table tbody tr.row-danger {
