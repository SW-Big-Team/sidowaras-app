...
<div class="row mb-4">
  <div class="col-12">
    <div class="search-bar-pro">
      <form method="GET" action="{{ route('kasir.transaksi.riwayat') }}" class="d-flex align-items-center gap-3 w-100">
        ...
        @if(request('start_date') || request('end_date') || request('metode'))
          <div class="active-filters">
            ...
            @endif
            <a href="{{ route('kasir.transaksi.riwayat') }}" class="clear-filters">
    ...
    <div class="card pro-card">
      <div class="card-header pro-card-header">
        ...
        <div class="header-right">
          <form method="GET" action="{{ route('kasir.transaksi.riwayat') }}" class="per-page-form">
          ...
          <table class="table pro-table mb-0">
            ...
            <tbody>
              ...
              <tr>
                <td>
                  <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="trx-info">
                      ...
                      <span class="trx-type">Penjualan Umum</span>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="time-info">
                    <span class="time-date">{{ $t->tgl_transaksi->format('d M Y') }}</span>
                    <span class="time-hour">{{ $t->tgl_transaksi->format('H:i') }} WIB</span>
                  </div>
                </td>
                <td>
                ...
                <td class="text-end">
                  <span class="amount-value">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</span>
                </td>
                <td class="text-center">
                  <a href="{{ route('kasir.transaksi.show', $t->id) }}" class="action-btn-link">
                    Detail
              ...
              <tr>
                <td colspan="5" class="text-center py-5">
                  <div class="empty-state">
                    ...
                    <p>Riwayat transaksi akan muncul di sini setelah Anda memproses pembayaran</p>
                  </div>
                </td>
              </tr>
              @endforelse
    ...
    <div class="modal-content modal-pro">
      ...
      <form method="GET" action="{{ route('kasir.transaksi.riwayat') }}">
        ...
        <div class="modal-footer-pro">
          <a href="{{ route('kasir.transaksi.riwayat') }}" class="btn-modal-secondary">
...
.filter-badge { background: white; color: var(--info); font-size: 0.65rem; padding: 2px 6px; border-radius: 10px; margin-left: 4px; }
...
.per-page-select:focus { border-color: var(--info); outline: none; }
...
.trx-icon-wrap i { font-size: 20px; color: var(--success); }
...
.action-btn-link:hover { background: var(--success); color: white; }
...
.date-input:focus { border-color: var(--info); outline: none; }
...
.method-option.active { border-color: var(--info); background: rgba(59,130,246,0.05); }
.method-option:hover { border-color: var(--info); }
...
.btn-modal-primary:hover { box-shadow: 0 4px 12px rgba(16,185,129,0.4); }
