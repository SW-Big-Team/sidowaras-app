        ...
        <div class="welcome-banner">
            <div class="welcome-content">
                <div class="welcome-text">
                    <span class="greeting-badge"><i class="material-symbols-rounded">add_shopping_cart</i> Pembelian Baru</span>
                    <h2 class="welcome-title">Tambah Pembelian</h2>
                    <p class="welcome-subtitle">Input data pembelian obat baru dari supplier.</p>
                </div>
...
<form action="{{ route('pembelian.store') }}" method="POST" id="formPembelian">
    ...
    <div class="card pro-card mb-4">
        <div class="form-section">
            <div class="section-header">
                <div class="section-icon primary"><i class="material-symbols-rounded">receipt_long</i></div>
            ...
            <div class="section-body">
                <div class="row g-3">
                    <div class="col-md-3 col-6">
                        ...
                        <div class="input-modern"><i class="material-symbols-rounded input-icon">tag</i><input type="text" name="no_faktur" class="form-control" value="{{ old('no_faktur') }}" placeholder="Auto"></div></div>
                    </div>
                    <div class="col-md-3 col-6">
                        <div class="form-group-modern"><label class="form-label-modern">Tanggal <span class="required">*</span></label>
                        <div class="input-modern"><i class="material-symbols-rounded input-icon">calendar_today</i><input type="datetime-local" name="tgl_pembelian" class="form-control" value="{{ old('tgl_pembelian', now()->format('Y-m-d\TH:i')) }}" required></div></div>
                    </div>
                    <div class="col-md-3 col-6">
                    ...
                    <div class="col-md-3 col-6">
                        ...
                        <div class="input-modern"><input type="text" id="total_harga_display" class="form-control total-field" value="Rp 0" readonly><input type="hidden" name="total_harga" id="total_harga" value="{{ old('total_harga', 0) }}"></div></div>
                    </div>
                </div>
                <div class="row g-3 mt-1">
                    <div class="col-md-6 col-12">
                        <div class="form-group-modern"><label class="form-label-modern">Supplier <span class="required">*</span></label>
                        <div class="input-modern select"><i class="material-symbols-rounded input-icon">local_shipping</i>
                    ...
                    <div class="col-md-6 col-12">
                        ...
                        <div class="input-modern"><i class="material-symbols-rounded input-icon">phone</i><input type="text" name="no_telepon_pengirim" class="form-control" value="{{ old('no_telepon_pengirim') }}"></div></div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3" id="containerObat">
                    @if(old('obat'))
                        @foreach(old('obat') as $i => $obat)
                            <script>window.oldObat = window.oldObat || [];window.oldObat.push(@json($obat));</script>
                        @endforeach
                    @endif
                </div>
            </div>
    ...
    <div class="card pro-card mb-4" id="cardTermin" style="display: none;">
        <div class="form-section">
            <div class="section-header">
                <div class="section-icon warning"><i class="material-symbols-rounded">calendar_month</i></div>
                <div><h6 class="section-title">Detail Termin Pembayaran</h6><p class="section-subtitle">Jadwal cicilan</p></div>
                <button type="button" class="btn-pro warning" id="btnTambahTermin"><i class="material-symbols-rounded">add</i> Tambah Termin</button>
            </div>
            <div class="section-body">
                <div id="containerTermin">
                    @if(old('termin_list'))
                        @foreach(old('termin_list') as $i => $termin)
                            <script>window.oldTermin = window.oldTermin || [];window.oldTermin.push(@json($termin));</script>
                        @endforeach
                    @endif
                </div>
            </div>
    ...
    <div class="form-actions">
        ...
        <button type="submit" class="btn-pro primary"><i class="material-symbols-rounded">save</i> Simpan Pembelian</button>
    </div>
</form>
@endsection
...
.btn-pro.primary { background: linear-gradient(135deg, #3b82f6, #1d4ed8); }
...
.obat-card:hover .card { border-color: var(--info) !important; box-shadow: 0 4px 12px rgba(59,130,246,0.1) !important; }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.oldObat && window.oldObat.length > 0) { window.oldObat.forEach(obatData => createObatCard(obatData)); } 
    else { createObatCard(); }
    
    if (window.oldTermin && window.oldTermin.length > 0) { window.oldTermin.forEach(terminData => createTerminCard(terminData)); }
    hitungTotal();
    toggleTerminCard();
});
</script>
